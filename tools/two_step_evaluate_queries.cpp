...
void evaluate_queries(
    ...
    const std::string& index_filename_2,
    ...
    std::string const& iteration)
{
    ...
    IndexType index2(MemorySource::mapped_file(index_filename_2));

    auto scorer = scorer::from_params(scorer_params, wdata);
    std::function<std::vector<typename topk_queue::entry_type>(Query, Query)> query_fun;
    ...
    tbb::parallel_for(size_t(0), queries.size(), [&, query_fun](size_t query_idx) {
        raw_results[query_idx] = query_fun(queries[query_idx], queries2[query_idx]);
    });
