...
void extract_times(
    ...
    std::vector<Query> const& queries2,
    ...
    std::ostream& os)
{
    ...
    for (auto&& [qid, query]: enumerate(queries)) {
        do_not_optimize_away(fn(query, queries2[qid],thresholds[qid]));
        std::generate(times.begin(), times.end(), [&fn, &q = query, &q2 = queries2[qid], &t = thresholds[qid]]() {
            return run_with_timer<std::chrono::microseconds>(
                       [&]() { do_not_optimize_away(fn(q, q2, t)); })
                .count();
        });
    ...
    for (size_t run = 0; run <= runs; ++run) {
        ...
        for (auto const& query: queries) {
            auto usecs = run_with_timer<std::chrono::microseconds>([&]() {
                uint64_t result = query_func(query, queries2[idx], thresholds[idx]);
                if (safe && result < k) {
                    num_reruns += 1;
                    result = query_func(query, queries2[idx], 0);
                }
...
void perftest(
    ...
    const std::string& index_filename2,
    const std::optional<std::string>& wand_data_filename,
    const std::vector<Query>& queries,
    const std::vector<Query>& queries2,
    ...
    bool safe)
{
    ...
    IndexType index2(MemorySource::mapped_file(index_filename2));
    ...
    std::function<uint64_t(Query, Query, Score)> query_fun;

    query_fun = [&](Query query, Query query2, Score) {
        topk_queue topk(k);
        ...
        topk2.finalize();
        return topk2.topk().size();
    };

    if (extract) {
        extract_times(query_fun, queries, queries2, thresholds, type, type, 2, std::cout);
    } else {
        op_perftest(query_fun, queries, queries2, thresholds, type, type, 2, k, safe);
    }
}
