...
namespace pisa { namespace scorer {
    inline auto from_params =
        ...
        if (params.name == "saturated_tf") {
            return std::make_unique<saturated_tf<std::decay_t<decltype(wdata)>>>(
                wdata, params.bm25_k1);
        }
        if (params.name == "qld") {
